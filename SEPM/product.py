...
# =================COLUMN 1===========================
        ...
        self.productTable=ttk. Treeview(p_frame, columns=("cid", "category", "supplier","name","price","qty","status"),yscrollcommand=scrolly.set,xscrollcommand=scrollx.set)
        ...
        self.productTable.column("cid",width=90)
...
# ======================================================================================
    ...
    def add(self):
        ...
        try:
            if self.var_cat.get()=="Select" or self.var_cat.get()=="Empty" or self.var_sup.get()=="Select" or self.var_sup.get()=="Empty" or self.var_name.get()=="":
                ...
                row=cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error","Product Already Exist, try different product",parent=self.root)
                else:
    ...
    def update(self):
        ...
        try:
            if self.var_cid.get()=="":
                messagebox.showerror("Error", "Please select product ID",parent=self.root)
            else:
                cur.execute("Select * from product where cid=?",(self.var_cid.get(),))
                row=cur.fetchone()
                if row==None:
                    messagebox.showerror("Error","Invalid Product ID",parent=self.root)
                else:
                    cur.execute("Update product set category=?,supplier=?,name=?,price=?,qty=?,status=? where cid=?",(
                                        self.var_cat.get(),
                                        self.var_sup.get(),
                                        self.var_name.get (),
                                        self.var_price.get (),
        ...
        try:
            if self.var_cid.get()=="":
                ...
                if row==None:
                    ...
                    if op==True:
                        cur.execute("delete from product where cid=?", (self.var_cid.get() ,))
                        con.commit ()
